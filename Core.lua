...
function FRM:ADDON_LOADED(addonName)
    if addonName ~= "FocusRollManager" and addonName ~= "LootManger-main" then return end
    self:InitDB()
end
...
function FRM:PrintHelp()
    ...
    self:Print("/frm sync [start|stop|push|status] - Live Sync zur Strichliste")
    self:Print("/frm reload - UI neu laden fuer externen Vollsync")
