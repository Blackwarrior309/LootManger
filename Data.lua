...
function FRM:UpsertPlayer(name, class, focus1, focus2, status, strikes)
    ...
    p.wins = p.wins or {}
    if self.BroadcastPlayer then self:BroadcastPlayer(name) end
end

function FRM:ClearFocus()
    local db = self:GetDB()
    db.players = {}
    if self.BroadcastClear then self:BroadcastClear() end
...
function FRM:MarkWin(player, item)
    ...
    if self.BroadcastAward then self:BroadcastAward(player, item, p.strikes) end
