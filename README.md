...
### Desktop-Tool mit einem Klick starten

Unter Windows die Datei `Start-FocusRollManagerDesk.bat` per Doppelklick starten.

Beim ersten Start wird automatisch eine lokale Python-Umgebung unter
`FocusRollManagerDesk/.venv/` erstellt und die benoetigten Pakete installiert.
Danach startet das Desktop-Tool direkt.
...
Wichtig: Der `.toc`-Dateiname muss zum Ordnernamen passen. Wenn der Ordner
`LootManger` heißt, muss darin `LootManger.toc` liegen. Wenn der Ordner
`FocusRollManager` heißt, muss darin `FocusRollManager.toc` liegen.
